...
So that I can inform other students of changes 
  ...
  Scenario: Student edits existing Event (Normal Flow)
     When the student requests to edit event JavaWorkshop with name PythonWorkshop
  ...
  Scenario: Student edits existing event without specifying at least one field (Error Flow)
     When the student requests to edit an existing event without specifying at least one field an error is shown
